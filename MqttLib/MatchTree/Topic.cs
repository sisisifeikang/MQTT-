  ...
  public class Topic
  {
    ...
    public string[] Levels
    {
      get { return levels; }
        ...
        {
            if (topicname.IndexOf('#') == -1 || topicname.IndexOf('#') == topicname.Length - 1)
            {
                levels = topicname.Split(TOPIC_SEPERATOR);
                foreach (string str in levels)
                {
                    if (str.Contains("#") && str != "#")
                        throw new ArgumentException("# is fixing with another string in one level");
                    if (str.Contains("+") && str != "+")
                        throw new ArgumentException("+ is fixing with another string in one level");
                }
            }
            ...
            {
                throw new ArgumentException("# is not the last char in topic name");
            }
